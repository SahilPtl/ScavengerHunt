...
try {
  ...
  if (
    ...
    fs.existsSync(".local/postgres/PG_VERSION")
  ) {
  ...
  if (occupied)
    throw Error(
      `Port ${config.port} is already serving HTTP. Run npm run stop, or choose PORT in server/.env.`,
    );
  ...
  fs.writeFileSync(".local/server-pid", String(child.pid));
  let ready = false;
  for (let i = 0; i < 30; i++) {
    if (child.exitCode !== null) break;
    await new Promise((r) => setTimeout(r, 500));
    ready = await fetch(url + "/api/health")
      .then((r) => r.ok)
      .catch(() => false);
    if (ready) break;
  }
  if (!ready) {
    child.kill();
    throw Error("API did not become healthy. Check database and port.");
  }
  console.log(
  ...
  child.on("exit", (code) => {
    try {
      fs.unlinkSync(".local/server-pid");
    } catch {}
